...
#
# This is the presenter's real bootstrap script for the "AI brain" setup shown
# in the 15:40 workshop segment. It is included so you can see how the whole
# three-system architecture (skills + knowledge hub + guardrails) boots from
# one idempotent script on a fresh machine.
#
...
# Deliberately not chmod +x for this reason.
# ============================================================================
set -euo pipefail

SKILLS_DIR="$HOME/.claude/skills"
HUB_DIR="$HOME/Developer/claude-work-hub"
...
write_skill() {
  ...
  if [ ! -f "$dir/SKILL.md" ]; then
    ...
    cat > /dev/null   # drain stdin so heredoc doesn't break
    echo "  exists  $1 (skipped)"
  fi
}

echo "==> Hub structure"
mkdir -p "$HUB_DIR"/{raw,wiki,scripts,templates/project-wiki}
...
write_skill "loop-candidates" << 'EOF'
---
...
write_skill "skill-to-validator" << 'EOF'
---
...
echo ""
echo "==> Cleanup orphan flat skill files"
for f in "$SKILLS_DIR"/*.md; do
  [ -f "$f" ] && rm "$f" && echo "  removed $(basename $f)"
done

echo ""
echo "Done. Run /reload-skills in Claude Code to pick up any new skills."
